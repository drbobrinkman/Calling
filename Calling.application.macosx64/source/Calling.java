...
//Play with XBox controller, using ControllerMate to create virtual mouse
 //Minim is the audio package for processing
...
public void draw() {  
    ...
    if(millis() - lastTrailMilli > 100){
      ...
      float proportion = ((mouseDist+prevSpeed)/2.0f - speedMin)/(speedMax-speedMin);
    ...
    if(state == 1){
      ...
      if(lastSuccess < mTowerParts2.length){
        ...
        if(oldPartX < mTowerParts2[lastSuccess].w && oldPartY < mTowerParts2[lastSuccess].h){
          ...
          int c = mTowerParts2[lastSuccess].img.get(oldPartX,oldPartY);
          if(alpha(c) > 0){
           lastSuccess += 1;
           bells[currentBell].rewind();
           bells[currentBell].play();
      ...
      if(lastSuccess < mTowerParts3.length){
        ...
        if(oldPartX < mTowerParts3[lastSuccess].w && oldPartY < mTowerParts3[lastSuccess].h){
          //Mouse is inside the picture, at least
          int c = mTowerParts3[lastSuccess].img.get(oldPartX,oldPartY);
          if(alpha(c) > 0){
           lastSuccess += 1;
           bells[currentBell].rewind();
           bells[currentBell].play();
...
public void playVideo(Movie mm){
  ...
  } else if(mm.time() < 1.5f){
    trans = 0; 
  } else if(mm.time() < 1.5f+3){
    trans = 255*(mm.time()-1.5f)/3;
  }
  ...
  float hscale = 1.0f;
  if(mm.width < 800){
    wscale = 800.0f/mm.width;
  }
  if(mm.height*wscale < 600){
    hscale = 600.0f/mm.height;
  }
...
public void endStateZero(){
  state = 1;
  //birds1.pause();
  myMovie.jump(myMovie.duration());
}

public void endStateTwo(){
  state = 3;
  speedMax = 20.0f;
  myMovie2.jump(myMovie2.duration());
}

public void endStateFour(){
  state = 5;
...
class Particle {
  ...
  Particle(PVector l, int icolor) {
    acceleration = new PVector(0,0.025f);

    float xdir = mouseX - lastMouseX;
    float ydir = mouseY - lastMouseY;
    
    velocity = new PVector(-xdir/15+random(-0.25f,0.25f),-ydir/15+random(-0.25f,0.25f));
    location = l.get();
    lifespan = 60;
    mColor = icolor;
  }
...
class ParticleSystem {
  ...
  public void addParticle(int xLoc,int yLoc, int ic) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--stop-color=#cccccc", "Calling" };
    if (passedArgs != null) {
